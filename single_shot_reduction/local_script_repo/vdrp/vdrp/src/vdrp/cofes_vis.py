...
def cofes_plots(ifunums, filename_array, outfile_name, vmin=-15,
    ...
    of fits files to plot. The output plot will be the shape of the
    input array.
    ...
    Pixels with vmin and below counts are black. Pixels with vmax and above
    counts are white.
    """
    filename_array = np.array(filename_array)
    assert filename_array.ndim < 3, "filename_array has more than two" \
        " dimensions. I can't plot that!"
    assert filename_array.size > 0, "filename_array has size zero. " \
        "There's nothing there to plot!"
    ...
    for i in np.arange(10):
        for j in np.arange(1, 11):
            ifuname = '%02d%01d' % (j, i)
            index = [ind for ind,
                     v in enumerate(ifunums) if ifuname == v]
            if len(index):
                f = filename_array[index[0]]
                ...
                try:
                    ...
                    msg = "cofes_plots: File {} not found. " \
                        "Skipping...".format(f)
                    if logging is not None:
                        logging.warning(msg)
                    else:
                        print(msg)
                    ax.imshow(np.zeros((49, 49)), vmin=1, vmax=2,
                              interpolation='nearest', origin='lower',
                              cmap=cmap)
...
def cofes_4x4_plots(prefix="", outfile_name='CoFeS_plots.png',
                    vmin=-15, vmax=25, logging=None):
    """
    ...
    # t1 = time.time()
    ifunums = \
        np.array([['012', '022', '032', '042', '052', '062', '072', '082', '092', '102'],
    ...
    ifunums = ['%02d%01d' % (j, i) for i in np.arange(10)
               for j in np.arange(1, 11)]
    ifunums.remove('010')
    ifunums.remove('011')
    ifunums.remove('012')
    ...
    assert len(cofes_files) > 0, \
        "There are no fits files that begin with " + "'" + \
    ...
    # filename_array = filename_array.reshape(ifunums.shape[0],
    #                                         ifunums.shape[1])
...
def main():
    ...
    parser.add_argument('prefix', type=str,
                        help='Prefix for the CoFeS files you wish to plot. '
                        'For example CoFeS20170202T061644.8')
    parser.add_argument('--output', default=None,
                        help='Output file name. By default this is '
                        'the given prefix.png')
    parser.add_argument('--vmin', type=float, default=-15,
                        help='Sets the lower level of stretch for '
                        'the output images.')
    parser.add_argument('--vmax', type=float, default=25,
                        help='Sets the upper level of stretch for '
                        'the output images.')
    args = parser.parse_args()

    if args.output is None:
        args.output = args.prefix + '.png'
    cofes_4x4_plots(prefix=args.prefix, outfile_name=args.output,
